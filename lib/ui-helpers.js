...
function createDownloadButtonForDespesas(getRowsFn, doc) {
  ...
  dlBtn.addEventListener('click', () => {
    ...
    a.click();
    setTimeout(() => { try { documentLocal.body.removeChild(a); URL.revokeObjectURL(url); } catch (e) {} }, 500);
  });
  return dlBtn;
}
...
function createPortalKeyModal(app, doc) {
  ...
  setTimeout(() => {
    ...
    const removeModal = () => { try { const m = documentLocal.querySelector('.modal-root'); if (m) m.remove(); } catch (e) {} };
    ...
    if (save) save.addEventListener('click', async () => {
      ...
      const key = v.trim();
      if (!key) return;
      try { localStorage.setItem('portal_api_key', key); } catch (e) {}
      ...
      try { if (app && typeof app.onPortalKeySaved === 'function') app.onPortalKeySaved(key); } catch (e) {}
    });
  }, 40);
  return modal;
}
