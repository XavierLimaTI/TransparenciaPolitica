...
// Try to load adapters if present
let camaraAdapter = null;
let senadoAdapter = null;
try { camaraAdapter = require('./adapters/camara'); } catch (e) { void e; }
try { senadoAdapter = require('./adapters/senado'); } catch (e) { void e; }
// Optional persistent cache (Node)
let persistentCache = null;
try { persistentCache = require('../server/cache'); } catch (e) { void e; }
...
class GovernmentAPI {
    ...
    async fetchData(base, endpoint, params = {}) {
        ...
        const cached = this.getCache(cacheKey) || (persistentCache ? persistentCache.get(cacheKey) : null);
        ...
        try {
            if (typeof window !== 'undefined' && typeof window.showGlobalSpinner === 'function') {
                try { window.showGlobalSpinner(`Carregando ${endpoint}...`); } catch (e) { void e; }
            }
            ...
            if (persistentCache && typeof persistentCache.set === 'function') {
                try { persistentCache.set(cacheKey, data, this.cacheTimeout); } catch (e) { void e; }
            }
            if (typeof window !== 'undefined' && typeof window.hideGlobalSpinner === 'function') { try { window.hideGlobalSpinner(); } catch (e) { void e; } }
            return data;
        } catch (error) {
            if (typeof window !== 'undefined' && typeof window.hideGlobalSpinner === 'function') { try { window.hideGlobalSpinner(); } catch (e) { void e; } }
    ...
    async fetchPortal(endpoint, params = {}) {
        ...
        while (attempt < maxAttempts) {
            attempt++;
            try {
                if (typeof window !== 'undefined' && typeof window.showGlobalSpinner === 'function') { try { window.showGlobalSpinner('Consultando Portal da Transparência...'); } catch (e) { void e; } }
    ...
    async getDeputadosAtuais() {
        ...
        if (!data) return [];
        if (camaraAdapter && typeof camaraAdapter.normalizeDeputadosApiResponse === 'function') {
            return camaraAdapter.normalizeDeputadosApiResponse(data).map(d => ({ ...d, ideologia: this.classificarIdeologia(d.partido) }));
        }
    ...
    async getSenadoresAtuais() {
        ...
        if (!data || !data.ListaParlamentarEmExercicio) {
            try { data = await this.fetchData('senado', '/senador/listaParlamentarEmExercicio'); } catch (e) { void e; }
        }
        if (!data) return [];
        if (senadoAdapter && typeof senadoAdapter.normalizeSenadoresApiResponse === 'function') {
            return senadoAdapter.normalizeSenadoresApiResponse(data).map(s => ({ ...s, ideologia: this.classificarIdeologia(s.partido) }));
        }
        if (!data.ListaParlamentarEmExercicio || !data.ListaParlamentarEmExercicio.Parlamentares) return [];
    ...
    async getDespesasPorParlamentar(params = {}) {
        ...
        const normalizeDespesaItem = (it) => {
            const dataDocumento = it.dataDocumento || it.data || it.dataEmissao || it.data || null;
            const descricao = it.descricao || it.historico || it.tipo || '';
            const valorRaw = it.valor || it.valorDocumento || it.valorPagamento || it.valor || 0;
            const valor = Number(String(valorRaw).replace(/\./g, '').replace(/,/g, '.').replace(/[^0-9.-]/g, '')) || 0;
            const favorecido = it.favorecido || it.nomeFavorecido || it.nome || '';
            const cnpjCpf = it.cpfCnpj || it.cpf || it.cnpj || null;
            const origem = it.orgao || it.unidade || null;
            return { dataDocumento, descricao, valor, favorecido, cnpjCpf, origem, detalhe: it };
        };

        const normalized = items.map(normalizeDespesaItem);
        return normalized;
    }
    ...
    loadDespesasFromCSV(text) {
        try {
            if (typeof module !== 'undefined' && module.exports) {
                try { const parser = require('../lib/csv-parser'); if (parser && typeof parser.parseDespesasCSV === 'function') return parser.parseDespesasCSV(text); } catch (e) { void e; }
            }
        ...
        const normalizeDespesaItem = (r) => {
            const dataDocumento = r.data || r.datadoc || r.data_documento || null;
            const descricao = r.descricao || r.historico || r.tipo || '';
            const valor = Number(String(r.valor || r.valor_documento || r.valor_pagamento || '0').replace(/\./g, '').replace(/,/g, '.').replace(/[^0-9.-]/g, '')) || 0;
            const favorecido = r.favorecido || r.nome || r.nome_favorecido || r.fornecedor || '';
            const cnpjCpf = r.cpf || r.cnpj || r.cpfcnpj || null;
            const origem = r.orgao || r.unidade || null;
            return { dataDocumento, descricao, valor, favorecido, cnpjCpf, origem, detalhe: r };
        };
        const normalized = rows.map(normalizeDespesaItem);
        return normalized;
    }
    ...
    useLocalDespesas(despesasArray) {
        ...
        try {
            if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function') {
                // Prefer window.CustomEvent so JSDOM/window-specific constructor is used in tests
                try {
                    const CE = (typeof window !== 'undefined' && window.CustomEvent) ? window.CustomEvent : (typeof CustomEvent !== 'undefined' ? CustomEvent : null);
                    if (CE) {
                        window.dispatchEvent(new CE('localDespesasUsed', { detail: { count: this._localDespesas.length } }));
                    } else {
                        // Fallback to a plain object (some non-browser test harnesses may accept this)
                        window.dispatchEvent({ type: 'localDespesasUsed', detail: { count: this._localDespesas.length } });
                    }
                } catch (e) { try { /* last-resort fallback */ window.dispatchEvent({ type: 'localDespesasUsed', detail: { count: this._localDespesas.length } }); } catch (__) { void __; } }
            }
    } catch (e) { void e; }
        ...
        try {
            if (typeof global !== 'undefined') {
                if (typeof global.dispatchEvent === 'function') {
                    ...
                    (global._events['localDespesasUsed'] || []).forEach(fn => { try { fn({ type: 'localDespesasUsed', detail: { count: this._localDespesas.length } }); } catch(_) { void _; } });
                }
            }
    } catch (e) { void e; }
    }
