...
// Normalize resource paths coming from manifest entries or CSV paths.
// Some manifest entries contain paths like "data/ingested/..." or already absolute
// '/resources/data/...'. This helper returns a safe absolute path that the
// frontend can fetch without producing '/resources/data/data/...'.
function normalizeResourcePath(p) {
    if (!p) return p;
    if (p.startsWith('/resources/')) return p;
    if (p.startsWith('/')) return p; // already absolute-ish
    if (p.startsWith('resources/')) return '/' + p.replace(/^\/*/, '');
    if (p.startsWith('data/')) return '/resources/' + p.replace(/^\/*/, ''); // data/ingested/... -> /resources/data/ingested/...
    if (p.startsWith('data\\')) return '/resources/' + p.replace(/^\\+/, '');
    // fallback: assume relative to resources/data
...
function criarFooterControles() {
    ...
    // Attach handlers to dataset loader controls if they exist (called from main.js)
    function attachDatasetLoaderControls() {
        if (typeof document === 'undefined' || typeof window === 'undefined') return;
        const btn = document.getElementById('loadLastNBtn');
        const input = document.getElementById('loadMonths');
        const status = document.getElementById('loadStatus');
        if (!btn || !input || !status) return;

        btn.addEventListener('click', async () => {
            const n = Number(input.value) || 3;
            status.textContent = 'Carregando...';
            try {
                // load manifest and pick last N months for 'despesas'
                if (typeof window.Datasets === 'undefined') { status.textContent = 'Datasets helper não disponível'; return; }
                const manifest = await window.Datasets.loadManifest();
                if (!manifest || !Array.isArray(manifest.files)) { status.textContent = 'Manifest não encontrado'; return; }

                // filter despesas entries and collect by month token (folder-like path)
                const despesas = manifest.files.filter(f => f.csv && f.csv.indexOf('despesas/') === 0);
                // group by month folder (e.g., despesas/20250901/...)
                const byMonth = {};
                despesas.forEach(f => {
                    const parts = f.csv.split('/');
                ...
                for (const m of months) {
                    ...
                    // prefer pago/favorecidos JSON if present — try to load each json path
                    for (const f of files) {
                        const path = f.json || f.path || ('/resources/data/ingested/' + f.csv.replace(/\//g, '__') + '.json');
                        try {
                            const data = await window.Datasets.fetchJSON(path);
                            if (Array.isArray(data)) aggregated = aggregated.concat(data);
                        } catch (e) { /* ignore per-file errors */ }
                    }
                }

                if (aggregated.length === 0) { status.textContent = 'Nenhum dado encontrado'; return; }
                // hand-off to governmentAPI
                if (window.governmentAPI && typeof window.governmentAPI.useLocalDespesas === 'function') {
                    window.governmentAPI.useLocalDespesas(aggregated);
                    window.dispatchEvent && window.dispatchEvent(new CustomEvent('localDespesasUsed', { detail: { count: aggregated.length } }));
                    status.textContent = `Carregado ${aggregated.length} registros`;
                } else {
                    status.textContent = 'API local não disponível';
                }
            } catch (err) {
                console.error('Erro no loader de datasets:', err);
...
async function initDatasetLoaderUI() {
    ...
    try {
        function normalizeResourcePath(p){
            ...
            if (p.startsWith('/resources')) return p;
            if (p.startsWith('resources')) return '/' + p;
            if (p.startsWith('data/')) return '/resources/' + p;
            return '/resources/data/' + p.replace(/^\/*/, '');
        }
        ...
        (manifest.files || []).forEach(f => {
            ...
            if (typeof f === 'string') { if (f === 'manifest.json' || f.endsWith('/manifest.json')) return; csvPath = f; jsonPath = normalizeResourcePath(f); label = f; }
            else if (typeof f === 'object') { csvPath = f.csv || ''; jsonPath = f.json ? normalizeResourcePath(f.json) : normalizeResourcePath(f.csv || ''); label = f.csv || f.json || JSON.stringify(f); }
            else return;
            const opt = document.createElement('option'); opt.value = jsonPath; opt.dataset.csv = csvPath;
            const count = (typeof f === 'object' && typeof f.count === 'number') ? f.count : null;
...
module.exports = Object.assign(module.exports, { showCandidatoDetails, showVotacaoDetails, criarFooterControles, initDatasetLoaderUI, attachDatasetLoaderControls, showProxyBanner, showLocalDataBanner });
