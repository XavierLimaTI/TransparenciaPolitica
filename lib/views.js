  ...
  const pageItems = (app.candidatosFiltrados || []).slice(start, end);
    ...
    // wire favorite click to app.toggleFavorito if present
        favBtn.addEventListener('click', (e) => {
            try { if (app && typeof app.toggleFavorito === 'function') app.toggleFavorito(candidato.id); } catch (err) { void err; }
            e.preventDefault();
        });
        ...
        verBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!key && window.governmentAPI && typeof window.governmentAPI.loadDespesasFromCSV === 'function') {
                    ...
                    try { window.dispatchEvent && window.dispatchEvent(new CustomEvent('localDespesasUsed', { detail: { count: 0 } })); } catch (e) { void e; }
                }
        ...
        list.innerHTML = (despesas || []).map(d => `
                ...
                if (!document.getElementById('downloadDespesasBtn')) {
                    ...
                    dlBtn.addEventListener('click', () => {
                        ...
                        document.body.appendChild(a); a.click(); setTimeout(() => { try { document.body.removeChild(a); URL.revokeObjectURL(url); } catch (e) { void e; } }, 500);
                    });
...
async function initDatasetLoaderUI() {
    ...
    try {
        ...
        try {
            ...
            if (found) { setTimeout(() => { try { select.value = found.value; loadBtn.click(); } catch (e) { void e; } }, 300); }
        } catch (e) { void e; }
...
function showLocalDataBanner() {
    ...
    b.appendChild(btn); b.appendChild(close); document.body.appendChild(b); setTimeout(() => { try { const el = document.getElementById('localDataBanner'); if (el) el.remove(); } catch (e) { void e; } }, 25000);
}
